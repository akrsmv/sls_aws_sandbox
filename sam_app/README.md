...
```bash
sam_app$ aws s3 mb s3://BUCKET_NAME
```
...
```bash
sam_app$ aws cloudformation describe-stacks \
    --stack-name sam_app \
...
```bash
sam_app$ dotnet test test/HelloWorld.Test
```
...
```bash
sam_app$ aws cloudformation delete-stack --stack-name sam_app
sam_app$ aws s3 rb s3://BUCKET_NAME
```
